...
with open(os.path.join(act_dir, '..', 'config', 'credentials.yml'), 'r') as f:
...
df_headcount = extract_to_sql(import_query(
    os.path.join(proyect_dir, 'sql', 'df_headcount.sql')), engine_2())

df_recording_log_30 = extract_to_sql(import_query(
    os.path.join(proyect_dir, 'sql', 'df_recording_log.sql')), engine_3())

df_recording_log_40 = extract_to_sql(import_query(
    os.path.join(proyect_dir, 'sql', 'df_recording_log.sql')), engine_5())
...
def load(df: pd.DataFrame):
    ...
    with engine_4() as con:
        ...
        try:
            ...
            df.to_sql('etiquetados_axa', con=con, if_exists='append',
            ...
            asyncio.run(enviar_mensaje(
                f'error al cargar el dataframe en la tabla: {e}'))
